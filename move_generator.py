...
class Generator:
    ...
    def make_a_move(self, move, side_to_move):
        ...
        print("my_pieces", self.my_pieces)
...
def create_moves(side_to_move, return_matrix=False, validate=True):
    ...
    if return_matrix:
        ...
        matrix = convert_to_matrix(side_to_move, sliding_attacks + other_attacks, defended_squares, check)
        return moves, matrix
    else:
        return moves
...
def convert_to_matrix(side_to_move, enemy_attacks=None, defended_squares=None, check=None):
    ...
    for rank_index, rank in enumerate(board):
        ...
        for square_index, square in enumerate(rank):
            ...
            #     number_of_defences = defended_squares.count(board[rank_index][square_index])
            #     square_list.append(round(number_of_defences/8 , 2))
            #     number_of_enemy_attacks = enemy_attacks.count(board[rank_index][square_index])
            #     square_list.append(round(number_of_enemy_attacks/8 , 2))
            # else:
...
class Search:
    def __init__(self):
        ...
        self.model = load_model('extra_input')
        self.hash_table = {}
        self.max_alpha = -100
        self.min_beta = 100
        self.steps = 0
