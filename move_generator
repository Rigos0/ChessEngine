...
class Generator:
    ...
    def go_back_to_previous_position(self, previous_position, side_to_move):
        ...
        self.b_short_castle = previous_position.get('b_short')
        self.b_long_castle = previous_position.get('b_long')
        self.w_short_castle = previous_position.get('w_short')
        self.w_long_castle = previous_position.get('w_long')
    ...
    def make_a_move(self, move, side_to_move):
        ...
        # delete taken pieces
        for x, y in enumerate(self.opponents_pieces[:5]):
            for index, position in enumerate(y):
                if position == move[1]:
...
class Search:
    ...
    def optipath_black(self):
        ...
        self.search_depth += 1
        if current_min > np.amin(self.zero_ply_predictions) or self.steps > self.max_steps or self.search_depth > 30:

            if current_min > np.amin(self.zero_ply_predictions):
                print(current_min, np.amin(self.zero_ply_predictions))
                self.search_depth = 0
            return current_min
        ...
        while True:
            ...
            if search_path_eval:
                for key in search_path_eval.keys():
                    ...
                    print(moves[np.argmin(node_predictions)])
            generator.go_back_to_previous_position(current_position, "black")
            if self.steps > self.max_steps:
                minimize = np.argmin(node_predictions)
                self.steps = 0
                return moves[minimize]
        ...
        while True:
            ...
            for key, value in search_path_eval.items():
                if value == "white":
                    ...
                    predictions[minimize_index] = key
                    generator.go_back_to_previous_position(current_position, "black")
